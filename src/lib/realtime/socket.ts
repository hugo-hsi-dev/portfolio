...
export type ConnectionState =
	'idle' | 'connecting' | 'open' | 'reconnecting' | 'offline' | 'closed';
...
export class BoardSocketController {
	...
	#handleOnline = (): void => {
		if (this.#intentionallyClosed || this.#state === 'open') return;
		this.#generation += 1;
	...
	#handleVisibility = (): void => {
		if (!this.#isVisible() || this.#intentionallyClosed || this.#state === 'open') return;
		this.#handleOnline();
	};
