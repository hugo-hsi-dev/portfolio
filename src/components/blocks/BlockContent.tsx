  ...
  if (!isExpanded) {
    return (
      <motion.div
        ...
        key={id}
        className={cn(
          `p-6 sm:p-8 md:p-10 lg:p-12 transition-shadow aspect-square ${highlightBlock && !isExpanded && 'ring-8 ring-offset-8 ring-fuchsia-500'} ${getColumns(cols)} ${getRows(rows)}`,
          className,
        )}
        layoutId={id}
        {...props}
